...
\begin{itemize}
	...
	\begin{itemize}
		\item Gradient: $\grad{\phi} = \frac{1}{h_{1}}\frac{\partial \phi}{\partial x^{1}}\hat{e}_{1} + \frac{1}{h_{2}}\frac{\partial \phi}{\partial x^{2}}\hat{e}_{2} + \frac{1}{h_{3}}\frac{\partial \phi}{\partial x^{3}}\hat{e}_{3}$
...
\item For a tensor $T^{i_{m}}_{j_{n}}$, we say it has a type $(m,n)$
\end{itemize}
...
\item To go from the dual space to the vector space we use the covariant metric $\mathfrak{g}^{ij}$ i.e. this raises the indices. Thus they are sometimes denoted using the symbol $\sharp$ and called "sharp operators"
